...
template<class T>
Array<T>::Array() : Array{detail::kDefaultCapacity} {} 

template<class T>
Array<T>::Array(std::nullptr_t) : Array{std::size_t{0}} {} 
...
template<class T>
Array<T>::Array(const Array &other) : Array{other.capacity_} { 
    if (buffer_ != nullptr) {
...
template<class T>
Array<T>::Array(Array &&other) noexcept : Array{nullptr} { 
    swap(other);
}
...
Array<T>::SizeType Array<T>::emplace(const SizeType index, Args &&... args) {
    assert_message(index <= size_, "index should not be greater than array size");
...
template<class T>
const typename Array<T>::Iterator Array<T>::Iterator::operator++(int) noexcept { 
    Iterator old{*this};
    operator++();
    return old;
}
...
template<class T>
const typename Array<T>::ConstIterator Array<T>::ConstIterator::operator++(int) noexcept { 
    Iterator old{*this};
    operator++();
    return old;
}
